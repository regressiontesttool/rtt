...
public abstract class AbstractParser
{
	...
	{
		public void scannerError(AbstractScanner.Exception e)
		{
	...
	public class TokenStream
	{
		...
		public TokenStream(AbstractScanner scanner)
		{
		...
		public TokenStream(AbstractScanner scanner, Symbol first_symbol)
		{
			this(scanner);
			enque(first_symbol);
		}
		...
		private Symbol readToken() throws IOException
		{
			while (true)
			{
				...
				catch (AbstractScanner.Exception e)
				{
	...
	public class Simulator
	{
		...
		{
			if (states == null || states.length < AbstractParser.this.states.length)
			{
				states = new short[AbstractParser.this.states.length];
				min_top = 0;
			}
			System.arraycopy(AbstractParser.this.states, min_top, states, min_top, (top = AbstractParser.this.top) + 1);
		}
	...
	protected AbstractParser(ParsingTables tables)
	{
		this.tables = tables;
	...
	*/
	public Object parse(AbstractScanner source) throws IOException, AbstractParser.Exception
	{
		init();
	...
	* @return semantic value of the accepted nonterminal
	*/
	public Object parse(AbstractScanner source, short alt_goal_marker_id) throws IOException, AbstractParser.Exception
	{
		init();
	...
	private Object parse(TokenStream in) throws IOException, AbstractParser.Exception
	{
		while (true)
		{
			Symbol token = in.nextToken();
	 ...
	 * @throws AbstractParser.Exception if Parser cannot recover
	 */
	protected void recoverFromError(Symbol token, TokenStream in) throws IOException, AbstractParser.Exception
	{
		if (token.id == 0) // end of input
			throw new AbstractParser.Exception("Cannot recover from the syntax error");
