...
/**
 * @production EqualityExpression : {@link BinaryExpression};
 * @ast node
 * @declaredat ./../../specifications/semantics/ast.ast:58
 */
public abstract class EqualityExpression extends BinaryExpression implements
		Cloneable {
	/**
	 * @apilevel low-level
	 */
	...
	public EqualityExpression clone() throws CloneNotSupportedException {
		EqualityExpression node = (EqualityExpression) super.clone();
		node.Type_visited = -1;
	 ...
	 * 
	 */
	public EqualityExpression(Expression p0, Expression p1) {
		setChild(p0, 0);
		setChild(p1, 1);
	}
	 ...
	 * @attribute syn
	 * @aspect TypeAnalysis
	 * @declaredat ./../../specifications/semantics/TypeAnalysis.jrag:96
	 */
	@SuppressWarnings({ "unchecked", "cast" })
	@Parser.Node.Compare
	public Type Type() {
	...
	private Type Type_compute() {
		return Type.bequals(getOperand1().Type(), getOperand2().Type())
				&& (getOperand1().Type().isReal() || getOperand1().Type()
						.isInteger()) ? Type.Boolean : Type.ERROR_TYPE;
	}
